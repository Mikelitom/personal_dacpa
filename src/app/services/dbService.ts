...
export const userService = {
  ...
  async getUsuarioById(id: string): Promise<Usuario | null> {
    ...
    if (error) {
      if (error.code === 'PGRST116') throw new Error(`Error fetching Usuario: ${error.message}`);
    }
  ...
  async getPadreFamiliaFromUsuario(email: string): Promise<PadreFamilia | null> {
    try {
  ...
  async getHistorialPago(): Promise<HistorialPago[]> {
    ...
    if (error) throw new Error(`Error fetching historial de pagos: ${error.message}`)
