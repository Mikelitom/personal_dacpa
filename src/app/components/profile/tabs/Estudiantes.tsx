    ...
    <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
      <div className="flex justify-center mb-4">
        <div className="w-32 h-32 rounded-lg overflow-hidden border-4 border-white shadow-md">
          <Image
            src={student.imagen || "/placeholder.svg"}
            alt={student.nombre}
            width={128}
            height={128}
      ...
      <h3 className="text-xl font-bold text-center text-gray-800 mb-2">{student.nombre}</h3>
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-gray-800 font-medium">{student.fechaNacimiento}</span>
        </div>
      </div>
    </div>
  )
...
function AcademicInfo({ student }: { student: StudentData }) {
  return (
    <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm h-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <AcademicInfoItem label="Ciclo Escolar" value="2023-2024" />
        <AcademicInfoItem label="Padre" value={student.padre} />
        <AcademicInfoItem label="Madre" value={student.madre} />
      </div>

      {student.convenio && (
        <ConventionInfo convenio={student.convenio} />
      )}
...
function ConventionInfo({ convenio }: { convenio: { tipo: string; descuento: string; vigencia: string } }) {
  return (
    <div className="mb-6">
      <h4 className="font-bold text-gray-800 mb-3 flex items-center">
        <svg
      ...
      <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
        <p className="font-medium text-gray-800">{convenio.tipo}</p>
        <p className="text-sm text-gray-600 mt-1">Descuento: {convenio.descuento}</p>
        <p className="text-sm text-gray-600 mt-1">Vigencia: {convenio.vigencia}</p>
      </div>
    </div>
  )
}